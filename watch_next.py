...
def next_movie(description):
    #Function finds similarity for each movie with description. Creates a dictionary with similarity as a key and the title of the movie 
    #as a value. Finds max vulue of similarity and returns respective movie.
    movie_similarirty = {}
    sentence = nlp(description)
    for movie in movies:
        similarity = nlp(movie).similarity(sentence)
...
print("Next Movie Recommended to Watch: " + next_movie(description))